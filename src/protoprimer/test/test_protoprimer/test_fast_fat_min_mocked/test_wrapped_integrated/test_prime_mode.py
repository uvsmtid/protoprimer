...
from test_protoprimer.test_slow_integrated import test_prime_mode


def test_relationship():
    assert_test_module_name_embeds_another_module_name(test_prime_mode.__name__)
...
def test_wrapped_prime_in_mock_env(fs: FakeFilesystem):
    ...
    with fat_mock_wrapper(fs):
        test_prime_mode.test_prime_mode(pathlib.Path(mock_test_dir.path))
