...
from test_protoprimer.test_slow_integrated import test_mode_prime_without_local_config


def test_relationship():
    assert_test_module_name_embeds_another_module_name(
        test_mode_prime_without_local_config.__name__
    )
...
def test_wrapped_prime_in_mock_env(fs: FakeFilesystem):
    ...
    with fat_mock_wrapper(fs):
        test_mode_prime_without_local_config.test_mode_prime_without_local_config(
