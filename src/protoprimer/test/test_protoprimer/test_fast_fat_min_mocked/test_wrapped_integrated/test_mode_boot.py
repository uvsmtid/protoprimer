...
from test_protoprimer.test_slow_integrated import test_mode_boot


def test_relationship():
    assert_test_module_name_embeds_another_module_name(test_mode_boot.__name__)
...
def test_wrapped_boot_in_mock_env(fs: FakeFilesystem):
    ...
    with fat_mock_wrapper(fs):
        test_mode_boot.test_mode_boot(pathlib.Path(mock_test_dir.path))
