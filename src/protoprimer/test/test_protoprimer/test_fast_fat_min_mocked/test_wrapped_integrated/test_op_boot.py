...
from test_protoprimer.test_slow_integrated import test_op_boot


def test_relationship():
    assert_test_module_name_embeds_another_module_name(test_op_boot.__name__)
...
def test_wrapped_boot_in_mock_env(fs: FakeFilesystem):
    ...
    with fat_mock_wrapper(fs):
        test_op_boot.test_op_boot(pathlib.Path(mock_test_dir.path))
