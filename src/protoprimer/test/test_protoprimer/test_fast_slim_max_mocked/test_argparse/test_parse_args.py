...
def test_parse_args_reinstall():
    # when:
    args = parse_args([CommandAction.action_reinstall.value])

    # then:
    assert (
        getattr(args, ParsedArg.name_run_mode.value)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["reinstall"],
            {
                ParsedArg.name_run_mode.value: "reinstall",
                SyntaxArg.dest_silent: False,
                SyntaxArg.dest_quiet: False,
                SyntaxArg.dest_verbose: 0,
            },
...
@pytest.mark.parametrize(
    "test_argv",
    [
        ["reinstall", "--final_state", "some_state"],
...
def test_main_help_contains_subcommand_descriptions():
    ...
    assert (
        normalize_space(
            "reinstall Re-create `venv`, re-install dependencies, and re-pin versions."
        )
        in normalized_output
    )
