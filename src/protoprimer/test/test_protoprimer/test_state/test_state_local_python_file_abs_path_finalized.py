...
# TODO: if `rel, it should not be named `abs` - see: ConfConstEnv.field_file_abs_path_python
def test_allow_rel_file_abs_path_python(
    ...
    mock_state_client_ref_dir_abs_path_global.return_value = "/abs/to/ref/dir"

    rel_path_to_python = "rel/path/to/python"

    mock_state_env_conf_file_data.return_value = {
        ConfConstEnv.field_file_abs_path_python: rel_path_to_python,
    }
    ...
    assert ret_val == os.path.join(
        mock_state_client_ref_dir_abs_path_global.return_value,
        rel_path_to_python,
    )
