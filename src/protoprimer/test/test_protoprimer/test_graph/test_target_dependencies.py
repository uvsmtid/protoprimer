...
from test_protoprimer.misc_tools.graph_utils import (
...
class TestTargetDependencies:
    """
    This test ensure there is no dangling states in the DAG which are not part of target states.
    """
    ...
    def test_all_env_states_are_dependencies_of_target_full_proto_bootstrap(
        ...
        target_state_name = TargetState.target_full_proto_bootstrap
        ...
        all_dependencies.add(target_state_name)
        ...
        assert not missing_dependencies, (
            ...
            f"[{target_state_name}]: {sorted(list(missing_dependencies))}"
        )
    ...
    def test_dependencies_of_state_run_mode_executed(
        ...
        env_context_instance = EnvContext()
        state_graph_instance = env_context_instance.state_graph
        target_state_name = TargetState.target_run_mode_executed
        ...
        all_dependencies_only.add(target_state_name)
