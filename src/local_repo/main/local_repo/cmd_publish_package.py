...
def publish_package(
    ...
    if get_command_code("git diff-index --quiet HEAD --") != 0:
        print("ERROR: uncommitted changes", file=sys.stderr)
        sys.exit(1)
    ...
    if not protoprimer_version:
        print(f"ERROR: Could not find version in {version_file_path}", file=sys.stderr)
        sys.exit(1)
    ...
    if re.match(r"^\d+\.\d+\.\d+\.dev\d+$", protoprimer_version):
        ...
        print(
            f"ERROR: unrecognized version pattern: {protoprimer_version}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        if is_dev_version:
            ...
            print(
                f"ERROR: current HEAD is not in {git_main_remote}/{git_main_branch}",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
