...
If you want (dev) users to run software from your `git` repo, try:
...
Ultimately, why not use `python` for itself?
...
*   `protoprimer` does not build a binary, stays as a text copy (`python` code) helper within a repo
...
TODO: combine philosophical sections together at the bottom

-->

In short, `shell` is a **deceptive trap**:
*   it entices through interactive CLI and familiarity (everyone knows it)
*   yet it is a very poor lang for evolving software
...
<!--
TODO: Remove the section in this form. Add section about support for monorepos.
-->
