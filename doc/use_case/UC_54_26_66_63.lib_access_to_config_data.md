...
There should be a way to access `protoprimer` config data like [reusable_dir][FT_20_13_95_11.reusable_dir.md]-s.

Other scripts must always start as `entry_script`-s
(see [entry_script][FT_75_87_82_46.entry_script.md]).

Using the data in the entry script means `protoprimer` acts as a `lib` with API to provide that data.

The access to the config data depends on [conf_leap][FT_89_41_35_82.conf_leap.md]
starting from the path to `proto_kernel`.

Normally, detection of the reliable path to `proto_kernel`
requires [isolated_python][FT_28_25_63_06.isolated_python.md]
which implies re-starting `python` with `-I` option.

## Solution

The entry scripts can avoid `python` re-start by providing the path to `proto_kernel`.
...
the [conf_leap][FT_89_41_35_82.conf_leap.md] without doing all the things it does for
