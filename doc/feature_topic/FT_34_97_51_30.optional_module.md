---
feature_topic: FT_34_97_51_30
topic_title: optional_module
topic_status: TODO
---


# FT_34_97_51_30.optional_module

TODO: Maybe the approach should be:
Use single module `proto_kernel` (when in `runtime_proto`) generated out of
multiple modules which `primer_kernel` imports (when in `runtime_meta`).

## Intro

Some functionality from [proto_kernel][FT_87_17_49_36.proto_kernel.md] may be optional.

Optional functionality is provided via optional modules
which are maintained the same way as the main `protoprimer.primer_kernel` module,
depending on the current [primer_runtime][FT_14_52_73_23.primer_runtime.md]:
*   during `runtime_proto`, import goes from a copy
*   during `runtime_meta`, import goes from the deployed package

For example, `eval` exec operation (see [effective_config][FT_19_44_42_19.effective_config.md])
is implemented via:
*   `proto_config.py` module during `runtime_proto`
*   `protoprimer.primer_config` module during `runtime_meta`

## CLI support

The `argparse` configuration can dynamically attempt to import optional modules and
either expose or not certain options to user.

## Min leaps shape

In the case of min leaps shape (see [tree_shape][FT_59_95_81_63.tree_shape.md]),
it should be optional to deploy a copy of optional modules together with the copy of `primer_kernel`.

## Approach to `import`

Such optional modules are imported relative to `proto_code` dir:
*   without running `python` with a module (still standalone script)
*   without turning `proto_code` dir into module directory with `__init__.py` files
*   without modifying `sys.path`

[FT_87_17_49_36.proto_kernel.md]: FT_87_17_49_36.proto_kernel.md
[FT_14_52_73_23.primer_runtime.md]: FT_14_52_73_23.primer_runtime.md
[FT_19_44_42_19.effective_config.md]: FT_19_44_42_19.effective_config.md
[FT_59_95_81_63.tree_shape.md]: FT_59_95_81_63.tree_shape.md
