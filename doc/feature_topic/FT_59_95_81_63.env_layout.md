---
feature_topic: FT_59_95_81_63
topic_title: env_layout
topic_status: TODO
---

The layouts below are configured via [FT_89_41_35_82.conf_leap.md][FT_89_41_35_82.conf_leap.md].

## min dir layout

```
^/                                      # repo root = ref root = script dir
│
├─ prime                                # copy of `protoprimer.primer_kernel`
└─ ...
   └─ ...
      └─ primer_kernel.json             # path `conf_proto` is pecified via env var
```

## max dir layout

```
^/                                      # repo root
│
├─ script_dir/
│  ├─ proto_kernel.py                   # proto_code = copy of `protoprimer.primer_kernel`
│  ├─ boot_env.py                       # custom wrapper for `proto_code`
│  ├─ conf_proto/
│  │  ├─ proto_kernel.conf_primer.json  # See FT_89_41_35_82.conf_leap.md / leap_primer
│  │  └─ ...
│  └─ ...
│
├─ client_dir/                          # ref root
│  ├─ gconf/
│  │  ├─ proto_kernel.conf_client.json  # See FT_89_41_35_82.conf_leap.md / leap_client
│  │  └─ ...
│  ├─ lconf/                            # symlink to (e.g.) ./dst/conf_default/
│  │  ├─ proto_kernel.conf_env.json     # same as (e.g.) ./env_dir/conf_default/proto_conf.env.json
│  │  └─ ...
│  ├─ dst/                              # dst ~ destination environments
│  │  ├─ conf_default/
│  │  │  ├─ proto_kernel.conf_env.json  # See FT_89_41_35_82.conf_leap.md / leap_env
│  │  │  └─ ...
│  │  ├─ conf_special/
│  │  │  ├─ proto_kernel.conf_env.json  # See FT_89_41_35_82.conf_leap.md / leap_env
│  │  │  └─ ...
│  │  └─ ...
│  └─ ...
└─ ...
```

## absolute min dir layout

This layout is not very useful as it relies on defaults only.

```
^/                                      # repo root = ref root = script dir
│
└─ prime                                # copy of `protoprimer.primer_kernel`
```

---

[FT_89_41_35_82.conf_leap.md]: FT_89_41_35_82.conf_leap.md
