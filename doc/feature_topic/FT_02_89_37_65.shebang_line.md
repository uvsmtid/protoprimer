---
feature_topic: FT_02_89_37_65
topic_title: shebang_line
topic_status: TODO
---


# FT_02_89_37_65.shebang_line

TODO: TODO_91_75_37_57.implement_shebang_update.md

## Intro

When [entry_script][FT_75_87_82_46.entry_script.md] starts,
it runs as [shebang line][shebang_wiki] instructs.

When [proto_kernel][FT_87_17_49_36.proto_kernel.md] is used as an entry script directly,
it is possible to configure shebang line when the `proto_kernel` copy is automatically updated.

## Primary use case

You can use standard `/usr/bin/env` command inside shebang to provide environment variables to
change the behavior of the script:

```
#!/usr/bin/env -S ENV_VAR_1=value_1 ENV_VAR_2=value_2 python
```

It is especially useful in the case of minimal [tree_shape][FT_59_95_81_63.tree_shape.md]
when the client code is kept clean of any extra config files.
This is the only way to tell `proto_kernel` a non-default path to the file with `leap_primer` conf
(see [conf_leap][FT_89_41_35_82.conf_leap.md]).

However, the approach is limited by the maximum size of the shebang line (practically, it is only 127 chars).

TODO: implement and update this doc with example
      Specify the env var

[FT_75_87_82_46.entry_script.md]: FT_75_87_82_46.entry_script.md
[FT_59_95_81_63.tree_shape.md]: FT_59_95_81_63.tree_shape.md
[FT_87_17_49_36.proto_kernel.md]: FT_87_17_49_36.proto_kernel.md
[FT_89_41_35_82.conf_leap.md]: FT_89_41_35_82.conf_leap.md
[shebang_wiki]: https://en.wikipedia.org/wiki/Shebang_(Unix)
