...
| Local config                               | Auth state                     | Outcome                                                                                      |
|--------------------------------------------|--------------------------------|----------------------------------------------------------------------------------------------|
| `extra-index-url` = private repo           | no auth / 401                  | warning only; falls back to PyPI — non-blocking                                              |
| `extra-index-url` = private repo           | keyring configured             | keyring runs non-interactively; `--no-input` disables interactive fallback                   |
| `extra-index-url` = private repo           | keyring prompts for passphrase | `--no-input` suppresses prompt → keyring fails → warning → falls back to PyPI — non-blocking |
| `index-url` = private repo (replaces PyPI) | 401                            | hard fail; `--no-input` prevents prompt but cannot prevent failure                           |

### Seed / "venv_driver": "venv_uv"

`uv` does **not** read `pip.conf`.
It reads `~/.config/uv/uv.toml` or env vars (`UV_EXTRA_INDEX_URL`, `UV_INDEX_URL`).
...
| Local config | Auth state | Outcome |
|---|---|---|
| `UV_EXTRA_INDEX_URL` = private repo | no auth / 401 | hard fail — does not fall back to PyPI |
| `UV_EXTRA_INDEX_URL` = private repo | keyring via subprocess | keyring called as subprocess; failure → hard fail |
| `UV_INDEX_URL` = private repo (replaces PyPI) | 401 | hard fail |
