---
feature_topic: FT_17_41_51_83
topic_title: private_artifact_repo
topic_status: TEST
---


# FT_17_41_51_83.private_artifact_repo

## Intro

`protoprimer` may need to install dependencies from private artifact repos.

This also requires authn/authz settings for those artifact repos.

There is a choice to rely on `python` (or `pip`, or `uv`, or ...) features to disvover
system or user configuration or make it configurable for that repo clone only.

Without taking away the choice, the most autonomous approach (isolated to that repo clone only) is required.

Isolation should allow hooks that configure access to these private artifact repos during bootstrap.

## `venv` restrictions

There are several things to take care of:
1.  private artifact repo URL
2.  private artifact repo authn/authz

The 1st one (URL) is easy -
it can be external (system or user) configuration or internal (via CLI arg specified by the wrapper).

The 2nd one (authn/authz) is harder -
it often requires additional packages in `venv` (e.g. for `pip`).
To avoid relying on system-level or user-level software,
those extra packages will have to be installed in the `venv`
before installing packages from private artifacts repos behind authn/authz.

## Local user config: seeded package upgrade

`protoprimer` seeds a fresh `venv` with packages (all exist on PyPI).

`--no-input` prevents hanging when a user local config introduces
a private repo that triggers an interactive credential prompt.

### Seed / "venv_driver": "venv_pip"

`pip` reads `~/.config/pip/pip.conf` (Linux) / `%APPDATA%\pip\pip.ini` (Windows).

*   `extra-index-url` = private repo, `keyring` configured:

    `keyring` succeeds non-interactively (`--no-input` ensures no interactive fallback).

*   `extra-index-url` = private repo, `keyring` prompts for passphrase:

    `--no-input` suppresses prompt, `keyring` fails, `pip` warns, `pip` falls back to PyPI - non-blocking.

*   `extra-index-url` = private repo, no auth / 401:

    `pip` warns only and falls back to PyPI - non-blocking.

*   `index-url` = private repo (replaces PyPI), 401:

    `pip` hard fails, `--no-input` prevents prompt but cannot prevent failure.

### Seed / "venv_driver": "venv_uv"

`uv` does **not** read `pip.conf`.

Instead, `uv` reads `~/.config/uv/uv.toml` or env vars (`UV_EXTRA_INDEX_URL`, `UV_INDEX_URL`).

`uv` is non-interactive by default - no `--no-input` equivalent needed.

`uv` treats index errors more strictly than `pip`:
a 401 on any configured index is typically a hard failure, not a warning.

*   `UV_EXTRA_INDEX_URL` = private repo, no auth / 401:

    `uv` hard fails - does not fall back to PyPI.

*   `UV_EXTRA_INDEX_URL` = private repo, `keyring` via `subprocess`:

    `keyring` called as `subprocess`, if `keyring` fails, `uv` hard fails.

*   `UV_INDEX_URL` = private repo (replaces PyPI), 401:

    `uv` hard fails.

If users have `uv` local config pointing to a private repo,
they must ensure auth works before `protoprimer` seeds the `venv`.

## A solution: Google Cloud artifact repo

It is testable via these manual steps:

*   Create (e.g.) `local_auth` project for editable installation.

*   The `local_auth` project should have the necessary dependencies like `keyring` (they should not require auth*).

*   Install that project first as a separate `install_group` (e.g. "install_auth").

*   Install others (requiring auth*) after that as another `install_group` (e.g. "install_main").

*   Add dependency on `dummy_private` (not used otherwise) to `local_repo`.

*   Modify config to use the two groups "install_auth" and "install_main" (see above).

*   Provide `--keyring-provider` in `extra_command_args`:

    *   In case of `venv_pip`, use `--keyring-provider` `import`:

        `pip` imports `keyring` from the active `venv` directly.

    *   In case of `venv_uv`, use `--keyring-provider` `subprocess`:

        `uv` calls the `keyring` CLI as a `subprocess`.

        `protoprimer` sets `PATH` to include the `venv` `bin` dir so `uv` can find it.

*   Provide `--extra-index-url` with URL in `extra_command_args`:

    Both `venv_pip` and `venv_uv` require the `oauth2accesstoken@` username prefix in the URL -
    this is the username hint that tells `keyrings.google-artifactregistry-auth`
    to provide access token as the password.

    For example (replace `/protoprimer/test-python-repo/` -> `/<project_name>/<artifact_repo_name/`):

    ```
    "https://oauth2accesstoken@asia-southeast1-python.pkg.dev/protoprimer/test-python-repo/simple"
    ```

### Install / "venv_driver": "venv_pip"

Config snippet for  `venv_pip` test:

```json
{
    "project_descriptors": [
        {
            "build_root_dir_rel_path": "src/local_auth",
            "install_extras": [],
            "install_group": "install_auth"
        },
        {
            "build_root_dir_rel_path": "src/local_doc",
            "install_extras": [],
            "install_group": "install_main"
        },
        {
            "build_root_dir_rel_path": "src/local_repo",
            "install_extras": [],
            "install_group": "install_main"
        },
        {
            "build_root_dir_rel_path": "src/local_test",
            "install_extras": [],
            "install_group": "install_main"
        },
        {
            "build_root_dir_rel_path": "src/metaprimer",
            "install_extras": [],
            "install_group": "install_main"
        },
        {
            "build_root_dir_rel_path": "src/protoprimer",
            "install_extras": [
                "test"
            ],
            "install_group": "install_main"
        }
    ],

    "install_specs": [
        {
            "install_auth": {
            }
        },
        {
            "install_main": {
                "extra_command_args": [
                    "--pre",
                    "--keyring-provider",
                    "import",
                    "--extra-index-url",
                    "https://oauth2accesstoken@asia-southeast1-python.pkg.dev/protoprimer/test-python-repo/simple"
                ]
            }
        }
    ]
}
```

### Install / "venv_driver": "venv_uv"

Config snippet for  `venv_uv` test:

```json
{
    "project_descriptors": [
        {
            "build_root_dir_rel_path": "src/local_auth",
            "install_extras": [],
            "install_group": "install_auth"
        },
        {
            "build_root_dir_rel_path": "src/local_doc",
            "install_extras": [],
            "install_group": "install_main"
        },
        {
            "build_root_dir_rel_path": "src/local_repo",
            "install_extras": [],
            "install_group": "install_main"
        },
        {
            "build_root_dir_rel_path": "src/local_test",
            "install_extras": [],
            "install_group": "install_main"
        },
        {
            "build_root_dir_rel_path": "src/metaprimer",
            "install_extras": [],
            "install_group": "install_main"
        },
        {
            "build_root_dir_rel_path": "src/protoprimer",
            "install_extras": [
                "test"
            ],
            "install_group": "install_main"
        }
    ],

    "install_specs": [
        {
            "install_auth": {
            }
        },
        {
            "install_main": {
                "extra_command_args": [
                    "--pre",
                    "--keyring-provider",
                    "subprocess",
                    "--extra-index-url",
                    "https://oauth2accesstoken@asia-southeast1-python.pkg.dev/protoprimer/test-python-repo/simple"
                ]
            }
        }
    ]
}
```

## Test private repo scenario

This is a sample test scenario to use private repo.

It publishes [dummy_private][dummy_private] into a private artifact repo.

Define the region and the project for Google Cloud:

```sh
export CLOUDSDK_COMPUTE_ZONE=asia-southeast1
export CLOUDSDK_CORE_PROJECT=protoprimer
export ARTIFACT_REPO_NAME=test-python-repo
```

Create an artifact repo:

```sh
./cmd/gcloud_artifact_repo create "${ARTIFACT_REPO_NAME}" --project="${CLOUDSDK_CORE_PROJECT}" --location="${CLOUDSDK_COMPUTE_ZONE}"
```

Publish package:

```sh
./cmd/publish_package dummy_private --no_tag --repository_url "https://${CLOUDSDK_COMPUTE_ZONE}-python.pkg.dev/protoprimer/${ARTIFACT_REPO_NAME}/"
```

See the content of the artifact repo (click on the `echo`-printed URL):

```sh
echo "https://console.cloud.google.com/artifacts/python/protoprimer/${CLOUDSDK_COMPUTE_ZONE}/${ARTIFACT_REPO_NAME}?project=protoprimer"
```

Set up a new test `venv`:

```sh
python -m venv tmp/test-venv
tmp/test-venv/bin/pip install keyring keyrings.google-artifactregistry-auth
```

NOTE: Several points:
*   `simple` suffix in the URL (otherwise, `pip` does not recognize the artifact repo structure).
*   `--pre` arg to allow pre-released packages (what was supposed to be published to the artifact repo).
*   `--extra-index-url` (instead of `--index-url`) to allow dependencies from other artifact repos.

Install package:

```sh
tmp/test-venv/bin/pip install dummy_private --pre --extra-index-url "https://${CLOUDSDK_COMPUTE_ZONE}-python.pkg.dev/protoprimer/${ARTIFACT_REPO_NAME}/simple"
```

Check package:

```sh
tmp/test-venv/bin/pip freeze
```

Delete artifact repo:

```sh
./cmd/gcloud_artifact_repo delete "${ARTIFACT_REPO_NAME}" --project="${CLOUDSDK_CORE_PROJECT}" --location="${CLOUDSDK_COMPUTE_ZONE}"
```

Delete the test `venv`:

```sh
mv tmp/test-venv tmp/test-venv."$( date -u +"%Y%m%dT%H%M%SZ" )"
```

[dummy_private]: https://github.com/uvsmtid/protoprimer/tree/main/src/dummy_private
