...
*   Provide `--keyring-provider` and `--extra-index-url` with URL in `extra_command_args`:

    *   in case of `venv_pip`, use `--keyring-provider` `import` (pip imports keyring from the active venv directly)
    *   in case of `venv_uv`, use `--keyring-provider` `subcommand` (uv calls the `keyring` CLI as a subprocess;
...
*   Both `venv_pip` and `venv_uv` require the `oauth2accesstoken@` username prefix in the URL –
    this is the username hint that tells `keyrings.google-artifactregistry-auth` to provide
    a Google access token as the password.
        ...
        },
        {
            "install_main": {
                "extra_command_args": [
                    ...
                    "import",
                    "--extra-index-url",
                    "https://oauth2accesstoken@asia-southeast1-python.pkg.dev/<project_name>/<repo_name>/simple"
                ]
            }
        }
    ]
