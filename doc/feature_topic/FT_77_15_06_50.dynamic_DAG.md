...
This eliminates complex `if/else` logic within `EnvState`-s
because they do not have to be overloaded for all possible targets.
...
*   `ExecOperation`: known only after CLI argument processing - see [exec_operation][FT_11_27_29_83.exec_operation.md]

TODO: FT_77_15_06_50.dynamic_DAG.md:
      `exec_operation` should affect how `GraphCoordinates` are set, but should not be a coordinate itself.
...
*   TODO: FT_77_15_06_50.dynamic_DAG.md:
          support different DAG depending on the `ExecOperation`.
