...
*   `state_name_proto` target is recognized in both `runtime_proto` and `runtime_neo`.
*   `state_name_neo` target is recognized only in `runtime_neo`.

In other words, whatever `runtime_neo` does (whatever names it has) is irrelevant in `runtime_proto`.
