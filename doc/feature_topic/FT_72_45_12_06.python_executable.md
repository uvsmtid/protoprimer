...
# FT_72_45_12_06.python_executable
...
<!--
TODO: TODO_50_98_96_87.support_windows.md: To support Windows, `os.execve` will have to be changed to use `subprocess` with a chain of children.
-->

## Details

That initial `python` executable, invoked via [entry_script][FT_75_87_82_46.entry_script.md], is unpredictable:
...
*   "selected" (about a **path** to executable):

    `protoprimer` selects `python` on the target system.

*   "required" (about a **version** of executable):

    `protoprimer` ensures required `python` version.

The behavior depends on the [venv_driver][FT_73_95_31_84.venv_driver.md]:

*   `uv`

    The selected `python` executable is an intermediate to **install** the required `python` version.

    `uv` installs the required `python` version instead of validating version of the selected `python`.

*   `pip`

    The selected `python` executable is used as is to **validate** the required `python` version.

    `pip` can install distribution packages but cannot install the required `python` version.
...
If `python` selector is not specified, or it returns no result,
`protoprimer` handles the basic (see below) by trying `python` basenames.
...
def create_venv_by_pip(given_python, required_version):
    # Verify `what_version(given_python)` matches the `required_version` prefix.
...
[FT_73_95_31_84.venv_driver.md]: FT_73_95_31_84.venv_driver.md
