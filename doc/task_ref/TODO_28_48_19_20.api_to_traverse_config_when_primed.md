...
*   Change `venv_shell` to use `start_app` (instead of `boot_env`) which accesses config.
*   Change `eval_conf` to use `start_app` (instead of `boot_env`) which accesses config.
